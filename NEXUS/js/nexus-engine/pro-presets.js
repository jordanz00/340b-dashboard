...
(function () {
  ...
  function build(showcase) {
    ...
    var seed = (NX.SessionSeed && NX.SessionSeed.getSeed) ? NX.SessionSeed.getSeed() : (Date.now() >>> 0);
    return shuffleSeeded(out, seed);
  }
  ...
  function apply(entry, options) {
    ...
    if (goScene && len) {
      var sk = (NX.SessionSeed && NX.SessionSeed.getSeed) ? (NX.SessionSeed.getSeed() >>> 0) : 0;
      NX.goNext((S.curS + 2 + (seed % 11) + (sk % 7)) % len);
    }
